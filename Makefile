...
package-darwin-arm64: lint pretest
	...
	mkdir -p ./bin
	cp ~/refactoring-agent/agent/target/aarch64-apple-darwin/release/refactoring-agent ./bin/refactoring-agent
...
package-darwin-x64: lint pretest
	...
	cd ~/refactoring-agent/agent && $(MAKE) build-all
	mkdir -p ./bin
	cp ~/refactoring-agent/agent/target/x86_64-apple-darwin/release/refactoring-agent ./bin/refactoring-agent
