...
export class CsRestApi {
  ...
  acceptRefactoring(fnToRefactor: FnToRefactor, traceId: string) {
    const config: AxiosRequestConfig = {
      headers: {
        'x-codescene-trace-id': traceId,
      },
    ...
    const conf = Object.assign({ headers: { Accept: 'application/json' } }, config);
    // we don't await, because we don't do anything with the result
    void this.axiosInstance.patch(this.refactorUrl(), this.refactorRequest(fnToRefactor), conf);
  }
}
